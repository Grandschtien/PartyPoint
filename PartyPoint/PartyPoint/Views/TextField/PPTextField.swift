...
final class PPTextField: UITextField {
    ...
    public enum DisplayState {
        ...
        func rightView() -> UIView? {
            ...
            case .default:
                return nil
            case .error(_):
                icon = Images.icInputError()
            case .success:
                icon = Images.icInputSuccess()
            }
    ...
    override var placeholder: String? {
        didSet {
            guard let placeholder = placeholder,
                  let color = Colors.mainColor()?.withAlphaComponent(0.5),
                  let font = Fonts.sfProDisplayBold(size: 14) else { return }
    ...
    private func updateStyle() {
        if style == .dark {
            borderView.backgroundColor = Colors.miniColor()
        } else {
            borderView.backgroundColor = Colors.miniColor()
        }
    }

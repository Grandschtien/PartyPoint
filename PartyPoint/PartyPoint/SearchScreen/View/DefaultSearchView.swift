...
private extension DefaultSearchView {
    ...
    func setupSearchIcon() {
        ...
        searchIcon.backgroundColor = Colors.mainColor()
        searchIcon.loopMode = .loop
    }
    
    func setupTitleLabel() {
        titleLabel.font = Fonts.sfProDisplayBold(size: TITLE_LABEL_FONT_SIZE)
        titleLabel.textAlignment = .center
        titleLabel.numberOfLines = numberOfRowsInTitle
    }
    
    func setupSubtitle() {
        subtitleLabel.font = Fonts.sfProDisplayRegular(size: SUBTITLE_LABEL_FONT_SIZE)

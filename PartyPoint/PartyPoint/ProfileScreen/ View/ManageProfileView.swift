...
final class ManageProfileView: UIView {
    ...
    func configure(configuration: ManageProfileConfiguration) {
        ...
        self.manageLabel.set(text: configuration.subtitle, rightIcon: Images.manage_chevron())
    }
}
...
private extension ManageProfileView {
    ...
    func setupSelf() {
        self.backgroundColor = Colors.miniColor()?.withAlphaComponent(0.1)
    ...
    func setupManageLabel() {
        manageLabel.font = Fonts.sfProDisplayMedium(size: MANAGE_LABEL_FONT_SIZE)
        manageLabel.textColor = Colors.manage_label_color()

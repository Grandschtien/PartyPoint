...
final class ProfileContentView: UIView {
    ...
    private var changePasswordAction: (() -> Void)?
    private var changeCityAction: (() -> Void)?
    ...
    private let exitButton = PPButton(style: .ghost(titleColor: PartyPointResourcesAsset.manageLabelColor.color), size: .s)
    ...
    func setChangePasswordAction(_ action: @escaping (() -> Void)) {
        changePasswordAction = action
    }
    
    func setChangeCityAction(_ action: @escaping (() -> Void)) {
    ...
    func setExitAction(_ action: @escaping (() -> Void)) {
...
private extension ProfileContentView {
    ...
    func setupExitButton() {
        exitButton.setTitle(PartyPointResourcesStrings.Localizable.exit, for: .normal)
        exitButton.addTarget(self, action: #selector(exitActionHandler), for: .touchUpInside)
    }
    
    func setupNavigationBar() {
        navigationBar.setTitle(PartyPointResourcesStrings.Localizable.profile)
    }
    
    func setupChangeCityView() {
        changeCityView.configure(configuration: .changeCity)
        changeCityView.addTapRecognizer(target: self, action: #selector(changeCityActionHandler))
    }
    
    func setupChangePasswordView() {
        changePasswordView.configure(configuration: .chnagePassword)
        changePasswordView.addTapRecognizer(target: self, action: #selector(changePasswordActionHandler))
    }

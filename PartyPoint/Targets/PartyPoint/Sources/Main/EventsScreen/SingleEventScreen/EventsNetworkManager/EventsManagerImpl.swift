...
extension EventsManagerImpl: EventsManager {
    func getTodayEvents(page: Int) async -> EventsManagerImpl.EventsStatus {
        ...
        guard let token = token else { return .error(reason:PartyPointResourcesStrings.Localizable.noToken) }
        guard let user = accountManager.getUser() else { return .error(reason:PartyPointResourcesStrings.Localizable.somthingGoesWrong)}

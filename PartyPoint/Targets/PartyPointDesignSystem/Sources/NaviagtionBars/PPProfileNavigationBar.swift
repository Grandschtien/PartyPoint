...
final class PPProfileNavigationBar: UIView {
    private var openProfileAction: (() -> Void)?
    ...
    func setOpenProfileAction(_ action: @escaping (() -> Void)) {
...
private extension PPProfileNavigationBar {
    ...
    func setupAvatarImageView() {
        ...
        avatarImageView.image = PartyPointResourcesAsset.profileImage.image
        avatarImageView.backgroundColor = PartyPointResourcesAsset.miniColor.color
        avatarImageView.clipsToBounds = true
    }
    
    func setupNameLabel() {
        nameLabel.font = PartyPointResourcesFontFamily.SFProDisplay.semibold.font(size: NAME_LABEL_FONT_SIZE)
        nameLabel.textColor = PartyPointResourcesAsset.miniColor.color
        nameLabel.numberOfLines = 1
    }
    
    func setupArrowImageView() {
        arrowImageView.image = PartyPointResourcesAsset.calendarChevron.image
    }
}

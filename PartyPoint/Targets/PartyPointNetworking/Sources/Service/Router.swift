...
public final class Router<EndPoint: EndPointType>: NetworkRouter {
    ...
    public func request(_ route: EndPoint) async -> (data: Data?, response: URLResponse?, error: Error?) {
    ...
    public func uploadRequest(_ route: EndPoint) async -> (data: Data?, response: URLResponse?, error: Error?) {
        let session = URLSession.shared
        do {
            let request = try self.buildRequest(from: route)
            let task = Task { () -> (Data?, URLResponse?, Error?) in
